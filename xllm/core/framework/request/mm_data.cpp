...
void MMData::get(uint32_t type, MMItemVec& items) const {
  if (!valid()) return;

  if (!hold<MMItemVec>()) return;

  const auto& vec = this->template items<MMItemVec>();
...
void MMData::get(const MMKey& key, std::vector<torch::Tensor>& items) const {
  if (!valid()) return;

  if (!hold<MMDict>()) return;

  const auto& dict = this->template items<MMDict>();
  const auto& itor = dict.find(key);

  if (itor == dict.end()) return;

  if (std::holds_alternative<torch::Tensor>(itor->second)) {
    items.push_back(std::get<torch::Tensor>(itor->second));
  } else if (std::holds_alternative<std::vector<torch::Tensor>>(itor->second)) {
    items = std::get<std::vector<torch::Tensor>>(itor->second);
  }
}
