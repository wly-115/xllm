...
bool EncoderInputGatherVisitor::finished(MMBatchData& mm_data) {
  MMDict dict;
...
bool EncoderOutputScatterVisitor::visit(MMDataItem& item) {
  ...
  if (item.type() == MMType::IMAGE) {
    ...
    LOG(ERROR) << " mm data item type invalid, type is" << item.type();
    return true;
  }
...
bool EncoderOutputScatterVisitor::finished() const {
  for (auto& [key, value] : data_) {
    ...
    if (name == "image") {
      ...
      LOG(ERROR) << "invalid modality key:" << key;
    }
...
bool EncoderEmbeddingGatherVisitor::finished(MMBatchData& mm_data) {
  MMDict data;
  torch::Tensor tar;
...
bool UpdateMMItemCachedStateVisitor::visit(MMDataItem& item) {
  ...
  auto& token_pos = item.mutable_state().mutable_token_pos();
