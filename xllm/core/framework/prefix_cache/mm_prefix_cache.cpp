...
void mm_murmur_hash3(const std::vector<const uint8_t*>& mm_hash_values,
                     ...
                     uint8_t* hash_value) {
  LOG(INFO) << "run mm murmur hash3" << mm_hash_values.size();
  ...
  uint8_t key[1024];
  memcpy(key, hash_value, MURMUR_HASH3_VALUE_LEN);
  // LOG(INFO) << "mm_murmur_hash3 3";
  uint8_t* p = key + MURMUR_HASH3_VALUE_LEN;
  // for(int  i = 0; i < MURMUR_HASH3_VALUE_LEN; ++i)
  //   {
  //     LOG(INFO) << (int64_t)mm_hash_values[0][i];
  //   }
  for (auto mm_hash_value : mm_hash_values) {
    memcpy(p, mm_hash_value, MURMUR_HASH3_VALUE_LEN);
    p += MURMUR_HASH3_VALUE_LEN;
  }
  // LOG(INFO) << "mm_murmur_hash3 1";
