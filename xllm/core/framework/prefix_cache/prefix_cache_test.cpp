...
void test_basic_operation(BlockManagerImpl* block_manager,
  ...
  {
    auto block_matched =
        prefix_cache->match(nullptr /*sequence*/, slice_token_ids);
  ...
  uint32_t n_blocks = token_ids.size() / block_size;
  {
    ...
    prefix_cache->insert(nullptr /*sequence*/, slice_token_ids, token_blocks);
  }
  ...
  {
    auto block_matched =
        prefix_cache->match(nullptr /*sequence*/,
...

  {
    auto block_matched = prefix_cache->match(
        nullptr /*sequence*/, slice_token_ids.slice(0, block_size));
    EXPECT_EQ(block_matched.size(), 1);
  }

  {
    auto block_matched = prefix_cache->match(nullptr /*sequence*/,
...
void test_insert_operation(BlockManagerImpl* block_manager,
  ...
  {
    auto block_matched =
        prefix_cache->match(nullptr /*sequence*/, slice_token_ids_1);
...

  {
    ...
    prefix_cache->insert(
        nullptr /*sequence*/, slice_token_ids_1, token_blocks_1);
...

  {
    ...
    prefix_cache->insert(nullptr /*sequence*/, slice_token_ids, block_matched);
  }
