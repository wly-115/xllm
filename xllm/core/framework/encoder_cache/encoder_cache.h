...
// Assumes each entry holds an independent storage (typical encoder output).
// If entries share storage via views, byte accounting may underestimate.
class EncoderCache {
 public:
  explicit EncoderCache(int64_t max_bytes);
 ...
 private:
  ...
  void evict_until_fit(int64_t incoming_bytes);
