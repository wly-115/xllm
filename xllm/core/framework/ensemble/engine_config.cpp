...
Options build_options(int32_t local_rank, int32_t world_size) {
  ...
  const bool is_local =
      !FLAGS_host.empty() &&
      net::extract_ip(FLAGS_master_node_addr) == net::extract_ip(FLAGS_host);
#if defined(USE_NPU)
...
NodeRuntimePlan build_node_runtime_plan(const GraphConfig& config,
  ...
  NodeRuntimePlan runtime_plan;
  runtime_plan.node_name = node.name;
  runtime_plan.backend = node.backend;
  ...
  runtime_plan.is_leader = node.ranks.begin()->first == global_rank;

  const std::vector<std::string>& downstream_names =
      config.get_downstream(node.name);
  CHECK_LE(downstream_names.size(), 1)
      << "P0 runtime plan supports at most one downstream node.";
  if (!downstream_names.empty()) {
    const std::string& downstream_name = downstream_names.front();
    const NodeConfig* downstream = config.find_node(downstream_name);
    CHECK(downstream != nullptr)
        << "Unknown downstream node: " << downstream_name;
    CHECK(!downstream->endpoint_target.empty())
        << "Downstream endpoint target cannot be empty: " << downstream_name;
    runtime_plan.downstream_node_name = downstream_name;
    runtime_plan.downstream_endpoint = downstream->endpoint_target;
  }
  return runtime_plan;
}
