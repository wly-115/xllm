...
torch::Tensor ImageUtils::resize(const torch::Tensor& image,
...
torch::Tensor ImageUtils::resize_video(const torch::Tensor& video,
                                       const std::vector<int64_t>& size,
                                       int resample,
                                       bool antialias) {
  auto origin_shape = video.sizes();
  auto reshaped_video =
      video.reshape({-1, origin_shape[2], origin_shape[3], origin_shape[4]});
  return resize(reshaped_video, size, resample, antialias)
      .reshape({origin_shape[0],
                origin_shape[1],
                origin_shape[2],
                size[0],
                size[1]});
...
torch::Tensor ImageUtils::centerCrop(const torch::Tensor& image,
...
torch::Tensor ImageUtils::rescale(const torch::Tensor& image, double scale) {
  return image * scale;
}

torch::Tensor ImageUtils::normalize(const torch::Tensor& image,
                                    const std::vector<double>& mean,
                                    const std::vector<double>& std) {
  // TODO: NCHW
  int numChannels = image.size(-3);
