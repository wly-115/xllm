...
class Qwen2_5_VLForConditionalGenerationImpl : public torch::nn::Module {
 public:
  ...
  torch::Tensor merge_multimodal_embeddings(
    ...
    int64_t n = is_multimodal.size(0);
    int64_t start = std::max(int64_t(0), n - 30);  // 防止 n < 50

    auto last_50 = is_multimodal.slice(0, start, n);
    // if(count) {
    LOG(INFO) << "merge_multimodal_embeddings";
    LOG(INFO) << "inputs_embeds: " << inputs_embeds.sizes();
    LOG(INFO) << "multimodal_embeds: " << multimodal_embeds.sizes();
    LOG(INFO) << "is_multimodal: " << is_multimodal.sizes();
    LOG(INFO) << last_50;
