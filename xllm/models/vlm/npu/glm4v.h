...
class GLM4VInputProcessor : public InputProcessor {
 ...
 public:
  ...
  void find_mm_spans(const std::vector<int>& prompt, MMData& mm_data) override {
    ...
    for (int idx = 0; idx < tokens_num; ++idx) {
 ...
 private:
  ...
  int image_start_token_id_;
  int image_end_token_id_;
  int video_start_token_id_;
  int video_end_token_id_;
  int image_token_id_;
