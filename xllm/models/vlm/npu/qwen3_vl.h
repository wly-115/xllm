...
class Qwen3_VLForConditionalGenerationImpl : public torch::nn::Module {
 public:
  ...
  MMDict get_multimodal_embeddings(const ModelInputParams& input_params) {
    ...
    if (image_input) {
      ...
      std::vector<int32_t> image_token_nums =
          get_mm_token_nums(input_params.mm_data, MMType::IMAGE);
      if (!use_encoder_dp_) {
    ...
    if (video_input) {
      ...
      std::vector<int32_t> video_token_nums =
          get_mm_token_nums(input_params.mm_data, MMType::VIDEO);
  ...
  torch::Tensor get_input_embeddings(const torch::Tensor input_ids,
    ...
    input_params.deep_stacks = std::move(deepstack_input_embeds);
    return inputs_embeds;
  }
