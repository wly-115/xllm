...
std::tuple<torch::Tensor, int32_t> Glm4VMPositionGenerator::generate(
  ...
  std::vector<std::tuple<std::string, int32_t, int32_t>> input_type_group =
      group_by_token_type(input_token_type);
  ...
  for (const std::tuple<std::string, int32_t, int32_t>& group :
       input_type_group) {
